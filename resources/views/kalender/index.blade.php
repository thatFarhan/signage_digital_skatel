@extends('layout.menu')
@section('content')
<div class="h-5/6 w-3/4 overflow-auto">
    <div class="flex items-end justify-between mb-5 h-[10%]">
        <div class="">
            <h1 class="font-bold text-2xl">Daftar Kalender Akademik</h1>
        </div>
        <a href="{{ route('kalender.create') }}"><div class="px-5 py-2 bg-black hover:bg-white text-white hover:text-black transition w-fit rounded-full border-[1.5px] border-black">Tambah Kegiatan</div></a>
    </div>
                    ...
                    <td class="border border-slate-400 px-5 py-2">
                        <div class="flex justify-center gap-2">
                            <div class="w-fit h-fit bg-green-400 hover:bg-green-500 transition p-2 rounded-md">
                                <a href="{{ route('kalender.edit', $d->id_akademik) }}" class="">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L6.832 19.82a4.5 4.5 0 0 1-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 0 1 1.13-1.897L16.863 4.487Zm0 0L19.5 7.125" />
                                    </svg>
                                </a>
                            </div>
                            <form onsubmit="return confirm('Yakin dihapus');" method="post" action="{{ route('kalender.destroy', $d->id_akademik) }}" class="h-[36px]">
                                @csrf
                                @method('DELETE')
                                <button type="submit">
